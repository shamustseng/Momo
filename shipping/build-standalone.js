...
html = html.replace('<script>\n/* ==================== 平台設定', () => `<script>window.SAMPLE_B64 = ${JSON.stringify(samples)};</script>\n<script>\n/* ==================== 平台設定`);
...
// 每個平台各出一個單一檔案：注入 window.PLATFORM
for (const [platform, outName] of [['momo', 'MOMO出貨單產生器.html'], ['cyberbiz', 'Cyberbiz出貨單產生器.html']]) {
  const out = path.join(here, outName);
  fs.writeFileSync(out, html.replace('<script>window.SAMPLE_B64 =', () => `<script>window.PLATFORM = ${JSON.stringify(platform)};</script>\n<script>window.SAMPLE_B64 =`));
